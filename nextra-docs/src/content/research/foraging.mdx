---
title: Grazing and Feeding Behaviors in Herbivores
description: This document summarizes research on grazing patterns, optimal foraging theory (Marginal Value Theorem), patch selection, feeding time budgets, and resource depletion responses in herbivorous mammals.
---
...
**Daily distance traveled varies significantly**:
- Sheep: 2.85 km/day
- Cattle: 1.68 km/day
- Horses: 1.65 km/day
...
**Critical Variables**:
- **Ti**: Time spent in a given patch (residence time)
- **Interpatch travel time**: Time moving between patches
- **Patch yield**: Amount of resources obtained
- **Intake rate**: Rate of resource acquisition within patch

**Key Predictions**:
- **Longer travel times between patches** = longer optimal giving-up times (GUTs)
- Leave patch when: **instantaneous intake rate = long-term average habitat intake rate**
- **Giving-Up Density (GUD)**: Quantifies forager's quitting harvest rate within a resource patch
...
- **Sheep** spend more time grazing smaller grass patches (1-6 m squared)
...
- **Longer grazing time**  to  higher intake  to  **longer rumination time** required for processing
...
| **Aspect** | **Cattle** | **Sheep** | **Horses** | **Deer** |
|------------|------------|-----------|------------|----------|
| **Grazing Time** | Moderate (bimodal) | Moderate (bimodal) | High (50-80%, continuous) | Variable |
| **Daily Distance** | 1.68 km | 2.85 km | 1.65 km | Not specified |
| **Soil Impact** | High compaction | Lower impact | High compaction | Low impact |
| **Forage Selection** | Grasses preferentially | More selective | Grasses preferentially | Highly selective |
| **Patch Preferences** | Medium-large patches | Small patches (1-6 m squared) | Medium-large patches | No consistent preference |
| **Digestive Strategy** | Ruminant | Ruminant | Hindgut fermenter | Ruminant |
...
- **Prefer smaller patches** (1-6 m squared)
...
- **No consistent grass patch size preferences** (1 m squared to larger areas)
...
```
Where dTi/dt = instantaneous intake rate in patch
...
5. **Mehdizadeh et al. (2023)** - "Classifying Chewing and Rumination in Dairy Cows"
6. **(2025)** - "Activity and behavior patterns of cattle, horses, and sheep grazing in mountainous areas"
7. **(2024)** - "Consistent individual differences in cattle grazing patterns"
8. **(2021)** - "Holling meets habitat selection: functional response of large herbivores revisited"
9. **Mezzalira et al. (2017)** - "Mechanisms and implications of a type IV functional response"

### Specialized Reviews

10. **Tedeschi (2019)** - "Mathematical modeling in ruminant nutrition: approaches" (78 citations)
11. **(2023)** - "Livestock feeding behavior: A tutorial review on automated techniques"
...
|-----------|---------------|-------------|
| `grazingStartTime` | 0-4000 | Morning grazing start (day ticks) |
| `grazingEndTime` | 10000-12000 | Evening grazing end (day ticks) |
| `middayRestStart` | 5000-6000 | Midday rest period start |
| `middayRestEnd` | 7000-8000 | Midday rest period end |
| `patchSize` | 3-10 blocks | Preferred grass patch size |
| `givingUpDensity` | 0.1-0.5 | When to abandon patch (0-1) |
| `biteSize` | 1-3 | Amount eaten per bite |
| `grazingSpeed` | 0.5-1.5 | Movement while grazing |
| `socialDominanceFactor` | 0.0-1.0 | Influence of dominance on patch access |

### Minecraft Entity Considerations

**Vanilla animals that could benefit**:
- **Cows, Sheep, Mooshroom**: Bimodal grazing, patch selection
- **Rabbits**: Selective grazing, creating grazing lawns
- **Horses (if added)**: Continuous grazing pattern
- **Goats**: Similar to sheep, more selective

**Behavior improvements**:
- Animals actually consume grass blocks over time (regenerate later)
- Animals move between grazing areas, not just wander randomly
- Dominant animals push subordinate animals from good grazing
- Pregnant/nursing mothers have higher nutritional needs
- Seasonal changes in grazing behavior (more grazing in "winter/spring" biomes)
