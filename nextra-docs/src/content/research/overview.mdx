---
title: Research Documentation
description: Better Ecology behaviors are based on academic research in animal ethology. This section documents the scientific foundation for each behavior implementation.
---

Better Ecology behaviors are based on academic research in animal ethology. This section documents the scientific foundation for each behavior implementation.

## Research Categories

### Movement and Social Behaviors

| Topic | Description |
|-------|-------------|
| [Herd Movement](/research/herd-movement) | Collective decision-making, leadership, quorum sensing |
| [Flocking](/research/flocking) | Boids algorithm, topological interactions, V-formation |
| [Parent-Offspring](/research/parent-offspring) | Attachment, bonding, imprinting, weaning |

### Survival Behaviors

| Topic | Description |
|-------|-------------|
| [Fleeing](/research/fleeing) | Flight initiation distance, escape strategies, panic |
| [Foraging](/research/foraging) | Marginal value theorem, patch selection, grazing |

### Predator-Prey Behaviors

| Topic | Description |
|-------|-------------|
| [Hunting](/research/hunting) | Predator strategies, prey selection, pack coordination |
| [Defensive Behaviors](/research/defense) | Armoring, chemical defenses, threat assessment |

### Population Dynamics

| Topic | Description |
|-------|-------------|
| [Spawning](/research/spawning) | Biome distribution, group sizes, territory spacing, carrying capacity |

### Life Cycle Behaviors

| Topic | Description |
|-------|-------------|
| [Breeding](/research/breeding) | Courtship, mate selection, territoriality |
| [Sleep](/research/sleep) | Activity cycles, rest patterns, vigilance |

## Key Academic References

The following foundational works inform the behavior implementations:

| Author | Year | Contribution |
|--------|------|--------------|
| Reynolds | 1987 | Boids algorithm for flocking |
| Charnov | 1976 | Marginal Value Theorem for foraging |
| Ydenberg and Dill | 1986 | Economic escape theory |
| Ballerini et al. | 2008 | Topological interactions in bird flocks |
| Couzin et al. | 2002 | Self-organized collective behavior |

## Implementation Approach

Research findings are translated into Minecraft implementations through:

1. **Parameter extraction**: Identifying key behavioral thresholds
2. **Algorithm design**: Creating simplified models of complex behaviors
3. **Balance tuning**: Adjusting for gameplay while maintaining scientific accuracy
4. **Configuration exposure**: Making parameters adjustable via data files

Each research document includes implementation notes specific to Minecraft entities.
